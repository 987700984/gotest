package model

import (
	"example.com/v2/dao"
	"fmt"
)

type Admin struct {
	Id       int64  `form:"id" gorm:"column:id;primary_key"` // 用户ID
	Username string `form:"username" gorm:"column:username"`
	Nickname string `form:"nickname" gorm:"column:nickname"`
	Password string `form:"password" gorm:"column:password"`
}

func (Admin) TableName() string {
	return "nt24_admin"
}

// 详情
func GetAdminTest(username string) (*Admin, error) {
	user := new(Admin)
	err := dao.Db.Where("username = ?", username).First(user).Error
	return user, err
}

// 添加
func CreateAdminTest(username string) (int64, error) {
	user := Admin{Username: username}

	fmt.Println(dao.Db)
	err := dao.Db.Create(&user).Error
	return user.Id, err
}

// 列表
func GetAdminList() ([]Admin, error) {
	var user []Admin
	err := dao.Db.Find(&user).Error
	return user, err
}

// 编辑
func EditAdminTest(id int, username string) {
	dao.Db.Model(&User{}).Where("id = ?", id).Update("nickname", username)
}

// 删除
func DelAdminTest(id int) error {
	var user User
	// DELETE from emails where id=20;
	//
	//dao.Db.Delete(&user, id)

	err := dao.Db.Where("id = ?", id).Delete(&user).Error
	return err
}
